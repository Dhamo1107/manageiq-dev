class MiqRequestController < ApplicationController
  ...
  #def pre_prov
  #@edit ||= {}
  #@edit[:hide_deprecated_templates] ||= false # Make sure this key exists
  #Rails.logger.debug "Edit Hash: #{@edit.inspect}"  
  #Rails.logger.debug "Search query: #{params[:search_query]}"
  #@search_text = params[:search_query]
  #if @search_text.present?
  #  @miq_requests = MiqTemplate.where("name LIKE ?", "%#{@search_text}%")
  #  Rails.logger.debug "Miq requests: #{@miq_requests}"
  #else
  #  @miq_requests = MiqTemplate.all
  #end
